...
generate_pw <- function(n, len) {
  ...
  pw <- sapply(seq_len(n), function(n) paste0(sample(chr, len, replace = TRUE), collapse = ""))
  return(pw)
}
...
.generate_users <- function(n, len) {
  ...
  password_hashed <- sapply(cli::cli_progress_along(password), function(i) sodium::password_store(password[i]))
  credentials_unmasked <- tibble::tibble(user_name, password, permissions = "standard")
  credentials_masked <- tibble::tibble(user_name, password_hashed, permissions = "standard")
...
write_unmasked <- function(x) {
  if(!dir.exists("unmasked")) dir.create("unmasked")
  filename_csv <- sprintf("unmasked/data_user_credentials_unmasked_%s.csv", format(Sys.Date(), "%d%m%y"))
  utils::write.csv(x$unmasked[, c("user_name", "password")], filename_csv, row.names = FALSE)
...
write_masked <- function(x) {
  cur_data <- unique(do.call(rbind, lapply(dir("csv", full.names = TRUE), utils::read.csv)))
  new_data <- rbind(cur_data, x$masked)
  if(any(duplicated(new_data$user_name))) cli::cli_abort("Data contains duplicated usernames. Please re-run the function!")
  filename_csv <- sprintf("csv/data_user_credentials_%s.csv", format(Sys.Date(), "%d%m%y"))
  utils::write.csv(new_data, filename_csv, row.names = FALSE)
  cli::cli_alert_success("Masked data written to csv-file {.file {filename_csv}}.")
  filename_sqlite <- sprintf("sqlite/db_credentials_%s.sqlite", format(Sys.Date(), "%d%m%y"))
  con <- DBI::dbConnect(drv = RSQLite::SQLite(), dbname = filename_sqlite)
  DBI::dbWriteTable(con, "credentials_db", new_data, row.names = FALSE, overwrite = TRUE)
  ...
  DBI::dbWriteTable(con, "credentials_db", new_data, row.names = FALSE, overwrite = TRUE)
  cli::cli_alert_success("Copyable version of the database written to {.file {f}}.")
  cli::cli_alert_info("You can use this file to directly for the server database")
  DBI::dbDisconnect(con)
}
